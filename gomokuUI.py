...
class GomokuGUI:
    ...
    def __init__(self, board_size=11):
        ...
        self.CircleRate = 0.45
        self.CrossRate = 0.45  # 十字的长度比率，暂时用不到
    ...
    def reset(self, bs):
        ...
        self.areas['SwitchPlayer'] = Rect(self.ScreenSize[0] / 2 - self.UnitSize * 1.5,
                                          self.ScreenSize[1] - self.UnitSize, self.UnitSize * 3, self.UnitSize)
    ...
    def render_step(self, action, player: int):
        """
        ...
        # 将上一次画的十字去掉（重新画一遍不带十字的）
        if self.last_action_player:
        ...
        # 显示步数与回合方
        self.step_counter += 1
        if player == 1:  # player.index传入的，1当前黑方，2当前白方
            self.show_messages(f'Step {self.step_counter}  black')
        else:
            self.show_messages(f'Step {self.step_counter}  white')

        pygame.display.update()

    def render_all_step(self, action, player: int, policy: dict, pi: list, v: list, min_max_stats: MinMaxStats):
        self._draw_static()  # 清空棋盘
        # 重新绘制所有棋子
        for a, p in self.state.items():
            self._draw_pieces(a, p)

        # 画最新的一步，有十字
        self.render_step(action, player)
    ...
    def draw_value(self, value: float, update=True):
        """
        展示由observation得到的network_output.value，注意调用时机。
        :param update:
        :return:
        """
    ...
    def _draw_pieces(self, loc, player, last_step=False):
        ...
        if type(loc) is int:
            ...
            x, y = loc
        pos = int(self.UnitSize * 1.5 + x * self.UnitSize), int(
            self.UnitSize * 1.5 + (self.BoardSize - y - 1) * self.UnitSize)
        if player == 1:
            c = (0, 0, 0)  # 黑色子
        elif player == 2:
            c = (255, 255, 255)  # 白色子
        ...
        pygame.draw.circle(self.screen, c, pos, int(self.UnitSize * self.CircleRate))
        if last_step:
            # 用红圈指示当前步
            pygame.draw.circle(self.screen, (255, 0, 0), pos, int(self.UnitSize * self.CircleRate), 2)
            ...
            #     c = (0, 0, 0)
            #
            # start_p1 = pos[0] - self.UnitSize * self.CrossRate, pos[1]
            # end_p1 = pos[0] + self.UnitSize * self.CrossRate, pos[1]
            # pygame.draw.line(self.screen, c, start_p1, end_p1)
            #
            # start_p2 = pos[0], pos[1] - self.UnitSize * self.CrossRate
            # end_p2 = pos[0], pos[1] + self.UnitSize * self.CrossRate
            # pygame.draw.line(self.screen, c, start_p2, end_p2)

    def _draw_pi(self, loc, p: float, pi: float, v: float):
        """画出p, π"""
        try:
            loc = int(loc)
        except Exception:
        ...
        pos = int(self.UnitSize * 1.5 + x * self.UnitSize), int(
            self.UnitSize * 1.5 + (self.BoardSize - y - 1) * self.UnitSize)
        c_pi_v = _get_color(v)  # 根据v变色实心填充访问率π
        pygame.draw.circle(self.screen, c_pi_v, pos, math.ceil(self.UnitSize * self.CircleRate * 0.8 * math.sqrt(pi)))
        c_p = (111, 111, 111)  # 绿色实线无填充概率p
        pygame.draw.circle(self.screen, c_p, pos, round(self.UnitSize * self.CircleRate * math.sqrt(p)), 1)  # 四舍五入
...
#     while True:
#         # 尝试显示P
#         for a, p in {Action(i): i / 122 for i in range(122)}.items():
#             UI._draw_pi(a.index, p, 1 - p, (p * 2 - 1))
#
#         if i == 1:
#             UI.show_messages(f'STEP:{5} BLACK')
#         else:
#             UI.show_messages(f'STEP:{6} WHITE')
#         inp = UI.get_input()
#         print(inp)
