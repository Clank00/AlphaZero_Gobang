...
def logging_initialize(run_name: str = None):
    # # Initialize tensorboard logging.
    if run_name is None:
        run_name = datetime.now().strftime("%Y%m%d-%H%M%S")

    log_dir = f"out/logs/" + run_name
...
class Monitor(ABC):
    ...
    def log(self, tensor: typing.Union[tf.Tensor, float], name: str) -> None:
        ...
        if self.reference.config.start_game_num % LOG_RATE == 0:
            tf.summary.scalar(name, data=tensor, step=self.reference.config.start_game_num)
    ...
    def log_distribution(self, tensor: typing.Union[tf.Tensor, np.ndarray], name: str) -> None:
        ...
        if self.reference.config.start_game_num % LOG_RATE == 0:
            tf.summary.histogram(name, tensor, step=self.reference.config.start_game_num)
